...
#[derive(Debug)]
struct Person { name: String, role: Option<String>, parent: Option<usize>, group: Option<usize> }
#[derive(Debug)]
struct Group { label: String }
#[derive(Debug)]
struct Org { people: Vec<Person>, groups: Vec<Group> }
    ...
    for line in input.lines() {
        ...
        if t.starts_with("group ") {
            ...
            if let Some(name) = rest.strip_suffix(" {") {
                let label = sq(name.trim()).to_string();
                let gid = groups.len();
        ...
        if t.starts_with("member ") {
            ...
            people.push(Person { name, role, parent: None, group: current_group });
            continue;
        }
        if t.contains(" -> ") {
...
const GROUP_PAD: f64 = 20.0; const GROUP_HEADER_H: f64 = 28.0; const GROUP_GAP_X: f64 = 40.0;
...
fn subtree_depth(people: &[Person], id: usize) -> usize {
    let children = children_of(people, id);
    if children.is_empty() { return 0; }
    children.iter().map(|&c| 1 + subtree_depth(people, c)).max().unwrap_or(0)
}
...
fn render_svg(org: &Org) -> String {
    let roots: Vec<usize> = org.people.iter().enumerate()
        .filter(|(_, p)| p.parent.is_none())
        .map(|(i, _)| i).collect();

    let has_groups = !org.groups.is_empty();
    ...
    if !has_groups {
        ...
        for &rid in &roots {
            ...
            render_node(&mut svg, &org.people, rid, root_x + sw / 2.0, PAD);
    ...
    // Grouped layout: compute each group's size, then ungrouped roots
    struct GroupLayout {
        group_idx: Option<usize>, // None = ungrouped
        roots: Vec<usize>,
        tree_w: f64,
        tree_h: f64,
    }

    let mut layouts: Vec<GroupLayout> = Vec::new();

    // Grouped members
    for (gi, _group) in org.groups.iter().enumerate() {
        let group_roots: Vec<usize> = roots.iter()
            .filter(|&&r| org.people[r].group == Some(gi))
            .copied().collect();
        if group_roots.is_empty() { continue; }

        let tree_w: f64 = group_roots.iter().map(|&r| subtree_width(&org.people, r)).sum::<f64>()
            + (group_roots.len().saturating_sub(1)) as f64 * NODE_GAP_X;
        let max_d = group_roots.iter()
            .map(|&r| subtree_depth(&org.people, r))
            .max().unwrap_or(0);
        let tree_h = (max_d + 1) as f64 * (NODE_ROLE_H + LEVEL_GAP_Y);

        layouts.push(GroupLayout { group_idx: Some(gi), roots: group_roots, tree_w, tree_h });
    }

    // Ungrouped roots
    let ungrouped: Vec<usize> = roots.iter()
        .filter(|&&r| org.people[r].group.is_none())
        .copied().collect();
    if !ungrouped.is_empty() {
        let tree_w: f64 = ungrouped.iter().map(|&r| subtree_width(&org.people, r)).sum::<f64>()
            + (ungrouped.len().saturating_sub(1)) as f64 * NODE_GAP_X;
        let max_d = ungrouped.iter()
            .map(|&r| subtree_depth(&org.people, r))
            .max().unwrap_or(0);
        let tree_h = (max_d + 1) as f64 * (NODE_ROLE_H + LEVEL_GAP_Y);
        layouts.push(GroupLayout { group_idx: None, roots: ungrouped, tree_w, tree_h });
    }

    // Compute total canvas size
    let max_tree_h = layouts.iter().map(|l| {
        if l.group_idx.is_some() { l.tree_h + GROUP_HEADER_H + GROUP_PAD * 2.0 } else { l.tree_h }
    }).fold(0.0_f64, f64::max);

    let total_w: f64 = PAD * 2.0 + layouts.iter().map(|l| {
        if l.group_idx.is_some() { l.tree_w + GROUP_PAD * 2.0 } else { l.tree_w }
    }).sum::<f64>() + (layouts.len().saturating_sub(1)) as f64 * GROUP_GAP_X;

    let total_h = PAD * 2.0 + max_tree_h;
    ...
    for (li, layout) in layouts.iter().enumerate() {
        if let Some(gi) = layout.group_idx {
            let group = &org.groups[gi];
            let group_w = layout.tree_w + GROUP_PAD * 2.0;
            let group_h = layout.tree_h + GROUP_HEADER_H + GROUP_PAD * 2.0;
            let (gbg, gstroke) = GROUP_COLORS[gi % GROUP_COLORS.len()];

            // Group background
            svg.push_str(&format!(
                "<rect x=\"{}\" y=\"{}\" width=\"{}\" height=\"{}\" rx=\"8\" fill=\"{}\" stroke=\"{}\" stroke-width=\"1.5\" stroke-dasharray=\"6,3\"/>",
                cursor_x, PAD, group_w, group_h, gbg, gstroke
            ));
            // Group label
            svg.push_str(&format!(
                "<text x=\"{}\" y=\"{}\" font-weight=\"bold\" font-size=\"13\" fill=\"{}\">{}</text>",
                cursor_x + 12.0, PAD + 20.0, gstroke, ex(&group.label)
            ));

            // Render trees inside group
            let inner_x = cursor_x + GROUP_PAD;
            let inner_y = PAD + GROUP_HEADER_H + GROUP_PAD;
            let mut root_x = inner_x;
            for &rid in &layout.roots {
                let sw = subtree_width(&org.people, rid);
                render_node(&mut svg, &org.people, rid, root_x + sw / 2.0, inner_y);
                root_x += sw + NODE_GAP_X;
            }

            cursor_x += group_w + GROUP_GAP_X;
        } else {
            // Ungrouped roots
            let mut root_x = cursor_x;
            for &rid in &layout.roots {
                let sw = subtree_width(&org.people, rid);
                render_node(&mut svg, &org.people, rid, root_x + sw / 2.0, PAD);
                root_x += sw + NODE_GAP_X;
            }
            cursor_x += layout.tree_w + GROUP_GAP_X;
        }
    }
...
Optionally group members with \"group \\\"Name\\\" { ... }\".

Example:
  group \"Client\" {
    member PM : \"Project Manager\"
    member Designer
    PM -> Designer
  }

  group \"Our Team\" {
    member TechLead : \"Tech Lead\"
    member FE : \"Frontend\"
    member BE : \"Backend\"
    TechLead -> FE
    TechLead -> BE
...
mod tests {
    ...
    fn parse_with_groups() {
        ...
        assert_eq!(o.groups[0].label, "A社");
        assert_eq!(o.people[0].group, Some(0));
        assert_eq!(o.people[2].group, Some(1));
    }
    ...
    #[test]
    fn render_grouped() {
        let input = "group \"Team A\" {\n  member X\n  member Y\n  X -> Y\n}\ngroup \"Team B\" {\n  member Z\n}\n";
        let o = parse(input).unwrap();
        let svg = render_svg(&o);
        assert!(svg.contains("Team A"));
        assert!(svg.contains("Team B"));
