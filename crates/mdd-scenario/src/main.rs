...
use mdd_layout::{Direction, LayoutConfig, LayoutEdge, LayoutElement, LayoutGraph, LayoutGroup, LayoutNode};
...
fn render_svg(diagram: &Diagram) -> String {
    ...
    // Render edges with step numbers
    ...
    for edge in &diagram.edges {
        ...
        // Step number + label
        let step_label = if edge.label.is_empty() {
            format!("{}", step_num)
        } else {
            format!("{}. {}", step_num, edge.label)
        };
        let (mx, my) = midpoint_on_path(&clipped);
        let lw = text_width(&step_label);
        svg.push_str(&format!(
        ...
        svg.push_str(&format!(
            ...
            mx, my, COLOR_EDGE, escape_xml(&step_label)
        ));
    }
...
const HELP: &str = "\
mdd-scenario - Render a system scenario as SVG (top-to-bottom flow)

Usage: mdd-scenario < input.txt

Same DSL as mdd-system, but laid out top-to-bottom to show
the flow of a specific scenario through system components.

Node types:
  process  Name          Circle (service, component)
  entity   Name          Double-bordered rectangle (external system)
  datastore Name { ... } Cylinder (database)
  actor    Name          Stick figure (person)
  file     Name { ... }  Dog-ear rectangle (document)
  queue    Name          Horizontal cylinder (message queue)
  data     Name { ... }  Dashed cylinder (non-persistent data)

Edges:
  From -> To : \"label\"
  From -> To : data \"Name\"          (inline data node)
  From -> To : data Name { ... }    (inline data with columns)
...
Example:
  ...
  ユーザー -> WebApp : \"ログインフォーム送信\"
  WebApp -> AuthService : \"認証リクエスト\"
  AuthService -> UserDB : \"ユーザー検索\"
  AuthService -> WebApp : data AuthToken {
    token
    expires_at
  }
  WebApp -> ユーザー : \"ダッシュボード表示\"
";
